...
public class G4Utils {
	...
	public static void copyPropFromBean2Dto(Object pFromObj, Dto pToDto) {
		if (pToDto != null) {
			try {
				pToDto.putAll(PropertyUtils.describe(pFromObj));
	 ...
	 *            Dto源对象
	 */
	public static void copyPropFromDto2Bean(Dto pDto, BaseVo vo) {
		if (pDto != null && vo != null) {
			try {
				BeanUtils.populate(vo, pDto);
			} catch (Exception e) {
				log.error("==开发人员请注意:==\n 将Dto对象值拷贝到JavaBean属性发生错误啦!" + "\n详细错误信息如下:");
				e.printStackTrace();
			}
		}
	}
