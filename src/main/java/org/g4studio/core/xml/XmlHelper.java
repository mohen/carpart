...
public class XmlHelper {
	...
	public static final String parseList2Xml2(List pList, String pRootNodeName, String pFirstNodeName) {
		Document document = DocumentHelper.createDocument();
		Element elRoot = document.addElement(pRootNodeName);
		for (int i = 0; i < pList.size(); i++) {
			BaseVo vo = (BaseVo) pList.get(i);
			Dto dto = new BaseDto();
			...
			while (it.hasNext()) {
				Dto.Entry entry = (Dto.Entry) it.next();
				Element nd = elRow.addElement((String) entry.getKey());
				nd.setText(String.valueOf(entry.getValue()));
			}
		}
		String outXml = document.asXML().substring(39);
		return outXml;
