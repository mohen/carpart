...
Ext.onReady(function() {
	...
	var cm = new Ext.grid.ColumnModel([rownum, {
		header : '停车场ID', // 列标题
		dataIndex : 'partId', // 数据索引:和Store模型对应
		hidden : true// 隐藏列
		},  {
		header : '停车场名字',
		dataIndex : 'name',
	...
	var store = new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
							url : 'carpart.do?reqCode=list'
						}),
				reader : new Ext.data.JsonReader({
							totalProperty : 'TOTALCOUNT', // 记录总数
							root : 'ROOT' // Json中的列表数据根节点
						}, [{
									name : 'partId' // Json中的属性Key值
								}, {
									name : 'name'
								}, {
									name : 'address'
	...
	function submitTheFormBasedBatch() {
		if (!myForm.getForm().isValid())
			return;
		myForm.form.submit({
					url : 'carpart.do?reqCode=batchSql',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
